function handleResult(result) {
	...
	for(let i = 0; i < result.length; i++) {
		genreListElement.append("<a href='movielist.html?id=" + result[i]["id"] + "&mode=browse&page=1'><li class='list-group-item'>" + result[i]["genre"] + "</li></a>");
	}
}
...
function handleAdvSearch(event) {
	...
	window.location.replace("movielist.html?"+url+"&mode=search&page=1");
}
